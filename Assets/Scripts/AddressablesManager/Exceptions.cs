...
namespace UnityEngine.AddressableAssets
{
    internal static class Exceptions
    {
        ...
        private const string _cannotLoadAssetKey = "Cannot find any asset path of type={0} by key={1}.";
        ...
        private const string _assetKeyNotSucceeded = "The asset with key={0} is not load success.";
        private const string _specifiedCastIsNotValid = "The asset with key={0} can not cast from type={1} to type={2}.";

        public static readonly InvalidKeyException InvalidReference = new InvalidKeyException("Reference is invalid.");

        public static string SpecifiedCastIsNotValid(string key, string fromType, string toType)
            => string.Format(_specifiedCastIsNotValid, key, fromType, toType);
        public static string AssetKeyNotSucceeded(string key)
            => string.Format(_assetKeyNotSucceeded, key);
        ...
        public static string CannotFindAssetKey<T>(string key)
            => string.Format(_cannotLoadAssetKey, typeof(T), key);
